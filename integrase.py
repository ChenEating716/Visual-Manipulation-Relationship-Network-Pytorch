...
class INTEGRASE(object):
    ...
    def single_step_perception_new(self, img, expr, cls_filter=None):
        ...
        if self.clue is not None:
            t_ground = self.mattnet_client(img, bboxes[:, :4].reshape(-1).tolist(), cls.reshape(-1).tolist(), self.clue)
            for i, score in enumerate(t_ground):
                obj_ind = ind_match_dict[i]
                if self.object_pool[obj_ind]["clue"] == self.clue:
                    # the clue has not been changed
        ...
        return bboxes, scores, rel_mat, rel_score_mat, leaf_desc_prob, ground_score, ground_result, ind_match_dict
