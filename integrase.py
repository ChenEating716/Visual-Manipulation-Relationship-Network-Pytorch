...
from model.utils.density_estimator import object_belief, gaussian_kde, relation_belief
...
class INTEGRASE(object):
    ...
    def _init_kde(self):
        ...
        with open("rel_density_estimation.pkl", "rb") as f:
            rel_data = pkl.load(f)
    ...
    def update_belief(self, belief, a, ans, data):
        ...
        # Q2
            ...
            print("current clue probability: ", t_ground)
            leaf_desc_belief[:, -1] = (t_ground[:, :-1] * leaf_desc_belief[:, :-1]).sum(-1)
    ...
    def bbox_post_process(self, bboxes, scores, rel_scores):
        prev_boxes = np.array([b["bbox"] for b in self.object_pool])
        det_to_pool = self.bbox_match(bboxes, prev_boxes)
        ...
        for i in not_matched:
            ...
            for j in range(len(self.object_pool[:-1])):
                if j in pool_to_det.keys():
                    new_rel = self._init_relation(rel_scores[:, i, pool_to_det[j]])
                    self.rel_pool[(det_to_pool[i], j)] = new_rel
    ...
    def multi_step_relation_det(self, rel_score_mat, ind_match_dict):
        ...
        for i in range(len(box_inds)):
            ...
            for j in range(i + 1, len(box_inds)):
                ...
                if (pool_ind_i, pool_ind_j) in self.rel_pool.keys():
                    rel_score = rel_score_mat[:, box_ind_i, box_ind_j]
                    self.rel_pool[(pool_ind_i, pool_ind_j)].update(rel_score, self.rel_kdes)
                else:
                    rel_score = rel_score_mat[:, box_ind_j, box_ind_i]
                    self.rel_pool[(pool_ind_j, pool_ind_i)].update(rel_score, self.rel_kdes)
                rel_prob_mat[:, box_ind_i, box_ind_j] = self.rel_pool[(pool_ind_j, pool_ind_i)].belief
        ...
        with torch.no_grad():
            ...
            leaf_desc_prob = leaf_and_descendant_stats(torch.from_numpy(rel_prob_mat) * triu_mask).numpy()
    ...
    def single_step_perception(self, img, expr, cls_filter=None):
        ...
        num_box = bboxes.shape[0]
        # relationship detection
        rel_mat, rel_score_mat = self.mrt_detection(img, bboxes)
        # update object and relationship pool
        ind_match_dict, not_matched = self.bbox_post_process(bboxes, scores, rel_score_mat)
        # multi-step relationship detection
        rel_prob_mat, leaf_desc_prob = self.multi_step_relation_det(rel_score_mat, ind_match_dict)
        ...
        return bboxes, scores, rel_mat, rel_prob_mat, leaf_desc_prob, ground_result, ind_match_dict
